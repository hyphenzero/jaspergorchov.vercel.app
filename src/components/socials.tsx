...
export function YouTubeIcon(props: React.ComponentPropsWithoutRef<'svg'>) {
  return (
    <svg fill="currentColor" viewBox="0 0 24 24" data-slot="icon" aria-hidden="true" {...props}>
      <path
        fillRule="evenodd"
...
export function BehanceIcon(props: React.ComponentPropsWithoutRef<'svg'>) {
  return (
    <svg fill="currentColor" viewBox="0 0 24 24" data-slot="icon" aria-hidden="true" {...props}>
      <path
        fill="currentColor"
...
export function Socials({ className }: { className?: string }) {
  return (
    <ul role="list" className={clsx('flex gap-x-3', className)}>
      {socials.map((profile) => (
        <li key={profile.name}>
          {/* <Link
            href={profile.href}
            aria-label={`Follow on ${profile.name}`}
            className="group -m-1 p-1"
          >
          	<profile.icon className="size-6 text-zinc-500 group-hover:text-zinc-950 sm:size-5 dark:group-hover:text-white" />
          </Link> */}
          <Button plain>
